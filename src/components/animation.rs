...
impl<'a> System<'a> for Animation {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        // for (e, pos, anim) in (&entities, &mut positions, &animations).join() {
        for (anim, e, pos) in (&animations, &entities, &mut positions).join() {
            if anim.start > now {
                // animation not started yet -> skip it
                // println!("[DEBUG] animation not started, waiting ...");
                continue;
            }
            ...
            if anim.loops > 0 && delta > anim.duration * anim.loops as u32 {
                ...
                // println!("[DEBUG] animation completed");
                pos.0 = anim.to;
                updater.remove::<MovementAnimation>(e);
                continue;
            }

            // updater.insert::<Position>(e, Position(animate(delta, anim)));
...
fn animate(delta: Duration, anim: &MovementAnimation) -> WorldPos {
    let MovementAnimation { duration, /*timing,*/ from, to, .. } = anim;
    let dt = (delta.as_nanos() % duration.as_nanos()) as f32
                / duration.as_nanos() as f32;
