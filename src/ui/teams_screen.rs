use crate::core::{Actor, DisplayStr, UserInput};
use crate::ui::{ClickArea, ClickAreas, FontFace, Scene, ScreenPos, ScreenText};
...
pub fn render(
    ...
    render_actors(&mut scene, &mut click_areas, actors);
    render_start_btn(
        &mut scene,
        &mut click_areas,
...
pub fn render_start_btn(
    ...
    let (w, h) = (230, 76);
    let (x, y) = (
        (viewport_width - w - 20) as i32,
        (viewport_height - h - 20) as i32,
    );

    scene.texts.push(
        // scene.texts[FontFace::Normal as usize].push(
        ScreenText::new(DisplayStr::new("Enter the arena ..."), ScreenPos(x, y))
            .padding(20)
